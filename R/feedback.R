#' JAJ feedback (with score)
#'
#' Here the participant is given textuak  feedback at the end of the test.
...
JAJ_feedback_graph_normal_curve <- function(perc_correct, x_min = 40, x_max = 160, x_mean = 100, x_sd = 15) {
  q <-
...
#'
#' Here the participant is given graphical feedback at the end of the test.
#' @param dict The psychTestR dictionary used for internationalisation.
#' @export
#' @examples
#' \dontrun{
#' demo_JAJ(feedback = JAJ_feedback_with_graph())}
#'
JAJ_feedback_with_graph <- function(dict = JAJ::JAJ_dict) {
  psychTestR::new_timeline(
    c(
      psychTestR::reactive_page(function(state, ...) {
        results <- psychTestR::get_results(state = state, complete = TRUE, add_session_info = FALSE)
